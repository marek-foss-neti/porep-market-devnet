...
without_zigzag_proofs() {
	env -u FIL_PROOFS_USE_ZIGZAG \
		-u FIL_PROOFS_ZIGZAG_GENERATE_MISSING_PARAMS \
		-u FIL_PROOFS_ZIGZAG_SIDECAR_DIR \
		-u FIL_PROOFS_PARENT_CACHE \
		-u FIL_PROOFS_USE_ZIGZAG_PARENT_CACHE \
		-u FIL_PROOFS_ZIGZAG_PARENT_CACHE_SIZE \
		-u FIL_PROOFS_SDR_PARENTS_CACHE_SIZE \
		"$@"
}
