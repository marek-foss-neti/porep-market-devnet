...
func zigzagDevnetUnseal(proof abi.RegisteredSealProof) bool {
...
func (t *TaskUnsealSdr) Do(taskID harmonytask.TaskID, stillOwned func() bool) (done bool, err error) {
	...
	if zigzagDevnetUnseal(sref.ProofType) {
		log.Infow("unseal skip sdr key for zigzag", "sector", sref.ID, "proof", sref.ProofType, "task", taskID)
	} else {
